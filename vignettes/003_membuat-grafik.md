...
ggplot(jabar_road, aes(x = district, y = road_length, fill = condition)) +
  geom_col(position = "dodge")+theme(axis.text = element_text(angle = 90))
```
...
``` r
ggplot(jabar_road, aes(x = district, y = road_length, fill = condition)) +
  geom_col()+theme(axis.text = element_text(angle = 90))
```
...
``` r
ggplot(jabar_road, aes(x = district, y = road_length, fill = condition)) +
  geom_col(position = "fill")+theme(axis.text = element_text(angle = 90))
```
...
ggplot(jabar_road, aes(x = district, y = road_length, fill = condition)) +
  ...
  scale_y_continuous(labels = scales::percent)+theme(axis.text = element_text(angle = 90))
```
...
```
    ...
    ## Reading layer `states_provinces' from data source `D:\Users\sai\Documents\dataviz\data-raw\states_provinces.json' using driver `GeoJSON'
